...
class Dataset(Dataset):
    ...
    def load(self):
        for id in range(len(self.episodes)):
            ...
            episode_content = {"context": [], "response": [], "knowledge_pool": [], "knowledge_piece_mask": [], "knowledge_label": [], 'Initiative_label': [], "node_id": []}
            for id_in_episode, example in enumerate(episode):
                ...
                # process for node_id (optional)
                node_id = example.get('node_id', -1)
                episode_content["node_id"].append(torch.tensor([node_id], requires_grad=False).long())
            ...
            while len(episode_content["context"]) < self.max_episode_length:
                ...
                episode_content['node_id'].append(torch.tensor([-1], requires_grad=False).long())
            ...
            assert len(episode_content["context"]) == len(episode_content["response"]) == len(
                episode_content["knowledge_pool"]) == len(episode_content["knowledge_piece_mask"]) == len(
                episode_content["knowledge_label"]) == len(episode_content["Initiative_label"]) == len(episode_content["node_id"]) == self.max_episode_length
            ...
            node_id_episode_tensor = torch.cat(episode_content["node_id"])  # [max_episode_length]


            self.episode_tensor.append(
                [id_episode_tensor, context_episode_tensor, response_episode_tensor, knowledge_pool_episode_tensor,
                 knowledge_piece_mask_episode_tensor, knowledge_label_episode_tensor, Initiative_label_episode_tensor, episode_mask, node_id_episode_tensor])
    ...
    def __getitem__(self, index):
        ...
        return [episode[0], episode[1], episode[2], episode[3], episode[4], episode[5], episode[6], episode[7], episode[8]]
...
def collate_fn(data):
    id_episodes, context_episodes, response_episodes, knowledge_pool_episodes, knowledge_piece_mask_episodes, knowledge_label_episodes, Initiative_label_episodes, episode_mask_episodes, node_id_episodes = zip(*data)
    ...
    return {'episode_id': torch.cat(id_episodes),  # [batch]
            ...
            'node_ids': torch.stack(node_id_episodes)  # [batch, max_episode_length]
    }
