...
class TeacherTopicShiftDiscriminator(nn.Module):
    def __init__(self, args=None, centrality_encoder=None):
        ...
        centrality_dim = 0 if centrality_encoder is None else centrality_encoder.output_dim
        ...
        self.project_c = nn.Linear(args.hidden_size, args.hidden_size)
        self.project_m = nn.Linear(args.hidden_size * 5 + centrality_dim, 2)

    def forward(self, context_pooling, knowledge_pooling_use, episode_mask, centrality_features=None, community_ids=None, mode="train"):
        # context_pooling [batch * max_episode_length, hidden_size]
        # knowledge_pooling_use [batch, max_episode_length, hidden_size]
        # episode_mask [batch, max_episode_length]
        ...
        fusion_parts = [gold_knowledge, context_pooling_pro, state, context_pooling_pro-state, context_pooling_pro*state]
        if self.centrality_encoder is not None:
            if centrality_features is None:
                centrality_features = torch.zeros(fusion_parts[0].size(0), 6, device=context_pooling.device)
            if community_ids is None:
                community_ids = torch.zeros(fusion_parts[0].size(0), dtype=torch.long, device=context_pooling.device)
            cent_embed = self.centrality_encoder(centrality_features, community_ids)
            fusion_parts.append(cent_embed)
        fusion = torch.cat(fusion_parts, dim=1)  # [batch * max_episode_length, hidden_size * 5 + centrality_dim]
...
class StudentTopicShiftDiscriminator(nn.Module):
    ...
    def forward(self, context_pooling, knowledge_pooling_use, episode_mask, centrality_features=None, community_ids=None, mode="train"):
        ...
        fusion_parts = [context_pooling_pro, state, context_pooling_pro-state, context_pooling_pro*state]
        if self.centrality_encoder is not None:
            if centrality_features is None:
                centrality_features = torch.zeros(fusion_parts[0].size(0), 6, device=context_pooling.device)
            if community_ids is None:
            ...
            cent_embed = self.centrality_encoder(centrality_features, community_ids)
            fusion_parts.append(cent_embed)
        fusion = torch.cat(fusion_parts, dim=1)  # [batch * max_episode_length, hidden_size * 4 + centrality_dim]
...
class TAKE(nn.Module):
    def __init__(self, vocab2id, id2vocab, args, centrality_loader=None):
        ...
        self.teacher_topic_shift_discriminator = TeacherTopicShiftDiscriminator(args=args, centrality_encoder=self.centrality_encoder)
        self.student_topic_shift_discriminator = StudentTopicShiftDiscriminator(args=args, centrality_encoder=self.centrality_encoder)
    ...
    def mixed_initiative_knowledge_selection_layer(self, data, encoded_state, epoch=0):
        ...
        if self.args.mode == "inference":
            s_shift_prob, s_state_pro = self.student_topic_shift_discriminator(
                context_vector, knowledge_pooling_use, data['episode_mask'],
                centrality_features=centrality_features, community_ids=community_ids, mode=self.args.mode
            )
        ...
        # Handle -1 values in dis (used for ignore_index at dialogue start)
        dis_safe = dis.clone()
        dis_safe[dis_safe < 0] = 0
        h_shift_prob = F.one_hot(dis_safe, 2) #hard_s_shift_prob  [batch * max_episode_length, 2]
