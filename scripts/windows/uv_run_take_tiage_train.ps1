...
# --- Activate virtual environment Python path ---
...
$args = @(
    ...
    "--train-batch-size","1"   # <= 这里改成 1
)

# --- Run Python directly ---
Write-Host "Starting TAKE training..."
# 使用 & 调用 python，并通过 Tee-Object 同时写日志和输出
& "$venvPython" @args 2>&1 | Tee-Object -FilePath $logFile -Append

# --- Check exit code ---
if ($LASTEXITCODE -ne 0) {
    Write-Error "Training failed with exit code $LASTEXITCODE. 查看日志: $logFile"
} else {
